...
public interface ICommandsManager<TCommand, TCommandAttribute, TArgumentAttribute>
	...
	Command<TCommand, TCommandAttribute, TArgumentAttribute>? GetCommand(string commandStringOrAlias);
	/// <summary>
	...
	bool TryGetCommand(string commandAlias, [NotNullWhen(true)] out Command<TCommand, TCommandAttribute, TArgumentAttribute>? command);
	/// <summary>
	...
	/// <returns> <see langword="true"/> if a <paramref name="command"/> is found, <see langword="false"/> otherwise. </returns>
	bool TryGetCommand(string commandString, [NotNullWhen(true)] out Command<TCommand, TCommandAttribute, TArgumentAttribute>? command, out string args);
}